...
export class Formula4Page {
  ...
  saveCalculation(note) {
    this.sqlite.create({
      ...
      db.executeSql("INSERT INTO saved_calculation (date, note, formula_id, formula_name, formula_page, var1, var2, var3) VALUES(strftime('%Y-%m-%d %H-%M-%S','now'),?,?,?,?,?,?,?)",[note, '4', 'EGS Pressure Calculation', 'Formula4Page', this.ata, this.fv, this.n])
        .then(res => {
          console.log(res);
